...
class Instruction_decoder(py4hw.Logic):
    def __init__(self, parent, name: str,
                 ...
                 # --- Reset (added for interface consistency across every
                 #     CPU component -- this block is purely combinational
                 #     via propagate(), so this only needs to clear the one
                 #     bit of cross-cycle state below, `lastins`) ---
                 ID_Reset=None,
                 ):
        ...
        self.reset = self.addIn('reset', ID_Reset) if ID_Reset is not None else None
