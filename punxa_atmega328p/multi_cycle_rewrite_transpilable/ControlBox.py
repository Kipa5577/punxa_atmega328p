...
class control_Box(py4hw.Logic):
    ...
    def __init__(self, parent, name,
                 ...
                 CB_InputSelect,            # To Datapath's BL register: selects whether it loads from the data bus or from the decoded K constant
                 CB_WE_Buffer,              # To Datapath: write-enable selecting which operand register (AL/AH/BL/BH/IO) is updated this cycle
                 ...
                 # FIX (SREG write race, see rewrite spec follow-up): pulses
                 # exactly one cycle -- the same cycle the merged 'done'
                 # (ib_done | irq_done) fires -- signalling that whichever
                 # leaf FSM handled this instruction has finished, meaning
                 # its ALU operands (AL/AH/BL/BH/IO) are guaranteed stable
                 # and this is the correct, single moment to commit the
                 # ALU's proposed SREG update. Datapath/MemoryInterfaceHandler
                 # gate the eSREG-driven flag write on this instead of
                 # applying it every cycle the instruction sits decoded,
                 # which was writing flag registers off stale/mid-load
                 # operands on early cycles and corrupting any flag a
                 # multi-operand op reads back as its own input (ADC/SBC's
                 # carry-in, ROL/ROR's carry-in, BLD's T-in).
                 CB_ALU_Commit,
                 CB_Reset=None,          # 1-bit IN: synchronous reset, fanned out to MainFSM,
                                         # INSTRUCTION_FSM_BOX, and InterruptFSM (all three as
                                         # direct siblings -- see InterruptFSM's docstring on why
                                         # it's not nested inside INSTRUCTION_FSM_BOX)
                 ):
        super().__init__(parent, name)
        ...
        self.CB_WE_Buffer              = self.addOut('CB_WE_Buffer', CB_WE_Buffer)                                    # Write-enable gating data transfer into Datapath's AL/AH/BL/BH/IO registers
        ...
        self.CB_ALU_Commit             = self.addOut('CB_ALU_Commit', CB_ALU_Commit)
        self.CB_Reset = self.addIn('CB_Reset', CB_Reset) if CB_Reset is not None else None
        ...
        # Bridging the merged 'done' back to MainFSM 'done' input -- this is
        # now the same wire exposed externally as CB_ALU_Commit (see param
        # docstring above), not a separate internal-only wire, so Datapath
        # can gate the SREG write on the exact same pulse.
        ...
        self.main_fsm = MainFSM( 
            ...
            MAIN_DONE=self.CB_ALU_Commit,   # Reads the internal (merged) done wire
